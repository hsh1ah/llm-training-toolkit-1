...
def chars_token_ratio(dataset, tokenizer, input_column_name, output_column_name, nb_examples=400):
    ...
    for _, example in tqdm(zip(range(nb_examples), iter(dataset)), total=nb_examples):
        text = example[input_column_name] + example[output_column_name]
...
class ConstantLengthDataset(IterableDataset):
    ...
    Iterable dataset that returns constant length chunks of tokens from stream of text files.
        Args:
            ...
            dataset: The dataset with text files.
            infinite: Whether the dataset should be reinitialized.
            seq_length: Length of token sequences to return.
            num_of_sequences: Number of token sequences.
            chars_per_token: Number of characters per token.
            content_column_name: Name of the column with content.
    """
    ...
    def __init__(
        ...
        self.concat_tokenizer = lambda texts: tokenizer(
        ...
        while more_examples:
            ...
            while True:
                if buffer_len >= self.max_buffer_size:
                    break
                try:
                    buffer.append(next(iterator)[self.content_column_name])
                    ...
                    more_examples = False
                    break
            tokenized_inputs = self.concat_tokenizer(buffer)
            for i in range(0, len(tokenized_inputs), self.seq_length):
                input_ids = tokenized_inputs[i : i + self.seq_length]
...
def create_datasets(tokenizer, args):
    ...
    if args.streaming:
        ...
        print(f"Size of train set: {len(train_data)}. Size of validation set: {len(valid_data)}")
    ...
    train_dataset = ConstantLengthDataset(
        ...
        content_column_name=args.output_column_name,
    )
